/**
 * Pure JavaScript 100% Offline ISO/IEC 18004 Standard QR Code Generator
 * Generates mathematically valid, scannable QR Code matrices for all camera scanners.
 */
(function() {
  ...
  QRCodeModel.prototype = {
    ...
    createSvgTag: function(cellSize, margin) {
      ...
      return '<svg width="' + fullSize + '" height="' + fullSize + '" viewBox="0 0 ' + fullSize + ' ' + fullSize + '" xmlns="http://www.w3.org/2000/svg"><rect width="' + fullSize + '" height="' + fullSize + '" fill="#ffffff" />' + rects + '</svg>';
    }
  };
  ...
  /**
   * Generates a 100% compliant ISO/IEC 18004 Standard SVG QR Code string
   * @param {string} text The URL or string to encode
   * @param {number} cellSize Pixel size per module (default 6)
   * @param {number} margin Module padding (default 4)
   * @returns {string} Fully rendered <svg> string
   */
  var root = (typeof window !== 'undefined' ? window : (typeof global !== 'undefined' ? global : this));
  root.QRCodeModel = QRCodeModel;
  root.generateStandardSvgQrCode = function(text, cellSize, margin) {
    if (!text) return '';
    try {
      cellSize = cellSize || 5;
      margin = (typeof margin === 'undefined') ? 4 : margin;
      var qr = new QRCodeModel(0, 1); // Auto type-number, Error Correction Level M
      qr.addData(String(text));
      qr.make();
      return qr.createSvgTag(cellSize, margin);
    } catch (e) {
      console.error("Standard QR generation error:", e);
      return '';
    }
  };
})();
